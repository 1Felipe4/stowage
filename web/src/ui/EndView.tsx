import { newRun, pressOn, retire } from '../engine/actions'
import { R } from '../engine/state'

export function EndView() {
  const s = R.summary,
    clear = R.over === 'clear'
  return (
    <>
      <div className={'orders ' + (clear ? 'go' : 'bad')}>
        <div className="ot">{clear ? `Stage ${R.stage} cleared.` : 'The run is over.'}</div>
      </div>
      <div className="endwrap">
        {s ? (
          <div className={'stamp ' + (s.profit >= 0 ? '' : 'bad')}>
            <h3>{s.profit >= 0 ? 'PROFIT ' + s.profit : 'LOSS ' + Math.abs(s.profit)}</h3>
            <div className="pl">
              <div>
                <dt>OPENED WITH</dt>
                <dd>{s.opening}</dd>
              </div>
              <div>
                <dt>REVENUE</dt>
                <dd className="up">+{s.revenue}</dd>
              </div>
              <div>
                <dt>SPENT ON SHIP &amp; FUEL</dt>
                <dd className="dn">−{s.spend - s.wages - s.penalty}</dd>
              </div>
              <div>
                <dt>WAGES</dt>
                <dd className="dn">−{s.wages}</dd>
              </div>
              {s.penalty ? (
                <div>
                  <dt>FORFEITS</dt>
                  <dd className="dn">−{s.penalty}</dd>
                </div>
              ) : null}
              <div>
                <dt>CLOSED WITH</dt>
                <dd>{R.credits}</dd>
              </div>
            </div>
            <div className="sub">
              BEST POSSIBLE HERE WAS {s.best} · {R.cleared} STAGE{R.cleared === 1 ? '' : 'S'} CLEARED
            </div>
          </div>
        ) : (
          <div className="stamp bad">
            <h3>BUST</h3>
            <p>{R.overWhy}.</p>
            <div className="sub">
              {R.cleared} STAGE{R.cleared === 1 ? '' : 'S'} CLEARED · {R.credits} CREDITS LEFT
            </div>
          </div>
        )}
        {clear ? (
          <>
            <button className="wide pri" style={{ marginTop: 12 }} onClick={pressOn}>
              PRESS ON TO STAGE {R.stage + 1}
            </button>
            <button className="wide" style={{ marginTop: 8 }} onClick={retire}>
              RETIRE ON {R.credits}
            </button>
          </>
        ) : (
          <button className="wide pri" style={{ marginTop: 12 }} onClick={newRun}>
            NEW HAULER
          </button>
        )}
      </div>
    </>
  )
}
