...
for (let st = 1; st <= 8; st++) {
  genStage(`AMB${st}`, st, null, HULLS[0])
  amb.push(R.ambient)
}
...
genStage('CLUSTER', 1, null, HULLS[0])
...
R.grid[5] = 'THR'
h = heatField(R.grid)
ck('a corner block is at the cap but still legal', Math.max(...R.grid.map((k,i)=>k?h[i]:-99)) === 5,
...
h = heatField(R.grid)
ck('spreading the same parts clears the cap', !R.grid.some((k, i) => k && h[i] > 5))
...
ck('a deck legal at ambient 0 can fail at ambient 3', okShallow && (!okDeep || atCap + 3 <= 5),
...
genStage('COLD', 5, null, HULLS[0])   // ambient 2
R.grid = new Array(20).fill(null)
...
genStage('LAYOUT', 7, null, HULLS[2])  // tug: 2 reactors, 3 engines, ambient 3
...
if (laid) {
  ...
  ck('that layout really is under the cap', !laid.some((k, i) => k && hh[i] > 5))
}
void NB
console.log(fail ? `\n${fail} FAILURES` : '\nALL PASS')
